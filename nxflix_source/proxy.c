...
#define CFG_DIR  "sdmc:/switch/NXFlix"
#define CFG_PATH "sdmc:/switch/NXFlix/proxy.cfg"
...
static int http_connect_tunnel(int fd, const ProxyConfig* cfg, const char* host, int port,
    ...
    if (send_all(fd, req, strlen(req)) < 0) {
        set_status(status, status_size, "HTTP PROXY SEND FAILED");
        return -1;
    }
    ...
    if (n <= 0) {
        set_status(status, status_size, "HTTP PROXY NO RESPONSE");
        return -1;
    }
    resp[n] = 0;
    if (!strstr(resp, " 200 ") && strncmp(resp, "HTTP/1.1 200", 12) != 0 && strncmp(resp, "HTTP/1.0 200", 12) != 0) {
        set_status(status, status_size, "HTTP PROXY CONNECT REJECTED");
        return -1;
    }
...
static int socks5_auth(int fd, const ProxyConfig* cfg, char* status, size_t status_size) {
    ...
    size_t hello_len = 0;
    hello[hello_len++] = 0x05;
    if (cfg->username[0]) {
        hello[hello_len++] = 0x02;
        hello[hello_len++] = 0x00;
        hello[hello_len++] = 0x02;
    } else {
        hello[hello_len++] = 0x01;
        hello[hello_len++] = 0x00;
    }
    if (send_all(fd, hello, hello_len) < 0) return -1;
    unsigned char reply[2];
    if (recv_all(fd, reply, 2) < 0 || reply[0] != 0x05 || reply[1] == 0xFF) {
        set_status(status, status_size, "SOCKS5 NEGOTIATION FAILED");
        return -1;
    }
    if (reply[1] == 0x02) {
        size_t ul = strlen(cfg->username);
        size_t pl = strlen(cfg->password);
        if (ul > 255 || pl > 255) return -1;
        unsigned char auth[520];
        size_t p = 0;
        auth[p++] = 0x01;
        auth[p++] = (unsigned char)ul;
        memcpy(auth + p, cfg->username, ul); p += ul;
        auth[p++] = (unsigned char)pl;
        memcpy(auth + p, cfg->password, pl); p += pl;
        if (send_all(fd, auth, p) < 0) return -1;
        unsigned char ar[2];
        if (recv_all(fd, ar, 2) < 0 || ar[1] != 0x00) {
            set_status(status, status_size, "SOCKS5 AUTH FAILED");
            return -1;
        }
    }
    return 0;
...
static int socks5_connect_tunnel(int fd, const ProxyConfig* cfg, const char* host, int port,
    ...
    req[p++] = 0x00;
    req[p++] = 0x03;
    req[p++] = (unsigned char)hl;
    memcpy(req + p, host, hl); p += hl;
    req[p++] = (unsigned char)((port >> 8) & 0xFF);
    req[p++] = (unsigned char)(port & 0xFF);
    if (send_all(fd, req, p) < 0) return -1;

    unsigned char head[4];
    if (recv_all(fd, head, 4) < 0 || head[0] != 0x05 || head[1] != 0x00) {
        set_status(status, status_size, "SOCKS5 CONNECT REJECTED");
        return -1;
    }
    size_t tail = 0;
    if (head[3] == 0x01) tail = 4 + 2;
    else if (head[3] == 0x04) tail = 16 + 2;
    else if (head[3] == 0x03) {
        unsigned char dl = 0;
        if (recv_all(fd, &dl, 1) < 0) return -1;
        tail = (size_t)dl + 2;
    } else return -1;
...
int proxy_connect_target(const ProxyConfig* cfg, const char* target_host, int target_port,
    ...
    const char* connect_host = target_host;
    int connect_port = target_port;
    if (cfg->mode != PROXY_MODE_DIRECT) {
        if (!cfg->host[0] || cfg->port <= 0) {
            set_status(status, status_size, "PROXY HOST OR PORT MISSING");
            return -1;
        }
    ...
    if (fd < 0) {
        set_status(status, status_size, cfg->mode == PROXY_MODE_DIRECT ? "DIRECT CONNECT FAILED" : "PROXY SERVER UNREACHABLE");
        return -1;
    }

    if (cfg->mode == PROXY_MODE_HTTP) {
        if (http_connect_tunnel(fd, cfg, target_host, target_port, status, status_size) < 0) {
            close(fd); return -1;
        }
    } else if (cfg->mode == PROXY_MODE_SOCKS5) {
        if (socks5_connect_tunnel(fd, cfg, target_host, target_port, status, status_size) < 0) {
            close(fd); return -1;
        }
    } else {
        set_status(status, status_size, "DIRECT TCP OK");
    }
    return fd;
}

bool proxy_test(const ProxyConfig* cfg, char* status, size_t status_size) {
    int fd = proxy_connect_target(cfg, "www.netflix.com", 443, 5000, status, status_size);
