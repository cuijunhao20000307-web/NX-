...
struct State{ProxyConfig proxy{};std::vector<AnimeItem> items;std::vector<size_t> view;AnimeDetail detail;Screen screen=SAFE;bool socket_ok=false,provider_ok=false,connected=false;int selected=0,ep=0;std::string search,status="READY - PRESS A TO CONNECT";};
...
static void header(u32* b,const std::string& page){u32 bg=rgba(9,10,14),panel=rgba(19,21,29),white=rgba(247,247,250),muted=rgba(148,153,168),pink=rgba(255,62,126);rect(b,g_stride,0,0,W,H,bg);rect(b,g_stride,0,0,W,88,panel);rect(b,g_stride,0,86,W,2,pink);text(b,40,57,36,"NXAnime",white,260);text(b,220,55,18,"GiriGiri",pink,180);text(b,1050,54,17,page,muted,180);}
...
static void draw_safe(u32* b,u32 s,const State& st){u32 bg=rgba(9,10,14),panel=rgba(20,22,30),white=rgba(244,245,248),muted=rgba(150,156,170),pink=rgba(255,62,126),green=rgba(55,205,120);rect(b,s,0,0,W,H,bg);rect(b,s,0,0,W,92,panel);rect(b,s,0,90,W,2,pink);safe_text(b,s,42,28,6,"NXANIME",white);safe_text(b,s,330,34,3,"HOS FONT BUILD 0.2.1",pink);safe_text(b,s,42,128,4,"GIRIGIRI PROVIDER",white);safe_text(b,s,42,174,2,"SAFE BOOT - LAZY TITLE CACHE",muted);rect(b,s,42,220,1196,90,panel);border(b,s,42,220,1196,90,2,st.connected?green:rgba(65,70,82));safe_text(b,s,68,245,3,st.connected?"SOURCE CONNECTED":"SOURCE NOT CONNECTED",st.connected?green:muted);char c[80];snprintf(c,sizeof(c),"CATALOG ITEMS: %zu",st.items.size());safe_text(b,s,68,280,2,c,white);rect(b,s,42,335,1196,180,panel);safe_text(b,s,68,362,3,"STATUS",pink);std::string a=st.status;for(char& x:a)if((u8)x>=128)x='?';safe_text(b,s,68,405,2,a.c_str(),white);char m[160];snprintf(m,sizeof(m),"PROXY MODE: %s",proxy_mode_name(st.proxy.mode));safe_text(b,s,68,455,2,m,muted);rect(b,s,42,540,1196,62,panel);safe_text(b,s,68,558,2,"A CONNECT",white);safe_text(b,s,330,558,2,"ZR PROXY",muted);safe_text(b,s,650,558,2,"Y TEST",muted);safe_text(b,s,1080,558,2,"+ EXIT",white);}

static void draw_catalog(u32* b,const State& st){header(b,"首页");u32 white=rgba(247,247,250),muted=rgba(148,153,168),pink=rgba(255,62,126),card=rgba(24,27,37),sel=rgba(37,40,53);text(b,40,132,25,"动漫目录",white,300);text(b,40,165,17,st.search.empty()?"GiriGiri 公共目录 · HOS 系统字体":("搜索："+st.search),muted,760);if(st.view.empty()){rect(b,g_stride,40,205,1200,360,card);text(b,75,285,28,"没有可显示的番剧",white,700);footer(b,st,"Y 刷新   + 退出");return;}int rows=7,start=st.selected>=rows?st.selected-rows+1:0;for(int r=0;r<rows;++r){int vi=start+r;if(vi>=(int)st.view.size())break;const auto& it=st.items[st.view[vi]];int y=190+r*62;bool on=vi==st.selected;rect(b,g_stride,40,y,1200,54,on?sel:card);if(on)rect(b,g_stride,40,y,6,54,pink);std::string title=(it.title.empty()?it.id:it.title);text(b,66,y+36,21,title,white,900);text(b,1040,y+34,15,it.id,on?pink:muted,170);}char c[64];snprintf(c,sizeof(c),"%d / %zu",st.selected+1,st.view.size());text(b,1080,615,15,c,muted,150);footer(b,st,"A 详情  X 搜索  Y 刷新");}
static void draw_detail(u32* b,const State& st){header(b,"详情");u32 white=rgba(247,247,250),muted=rgba(148,153,168),pink=rgba(255,62,126),card=rgba(24,27,37),sel=rgba(37,40,53);text(b,40,135,29,st.detail.title,white,1170);text(b,40,170,17,st.detail.status.empty()?st.detail.id:(st.detail.status+" · "+st.detail.id),pink,1000);rect(b,g_stride,40,195,1200,125,card);text(b,62,228,17,st.detail.description.empty()?"暂无简介":st.detail.description,muted,1140,4);text(b,40,360,21,"选集",white,200);if(st.detail.episodes.empty())text(b,40,410,19,"没有解析到公开集数入口。",muted,760);else{const int cols=8;int start=(st.ep/24)*24;for(int i=0;i<24;++i){int ei=start+i;if(ei>=(int)st.detail.episodes.size())break;int x=40+(i%cols)*149,y=392+(i/cols)*70;bool on=ei==st.ep;rect(b,g_stride,x,y,136,58,on?sel:card);border(b,g_stride,x,y,136,58,on?3:1,on?pink:rgba(62,66,82));text(b,x+14,y+37,17,st.detail.episodes[ei].label,on?white:muted,108);}}footer(b,st,"A 选择   B 返回");}
static void draw_episode(u32* b,const State& st){header(b,"播放");u32 white=rgba(247,247,250),muted=rgba(148,153,168),pink=rgba(255,62,126),card=rgba(24,27,37);std::string label="集数",url;if(!st.detail.episodes.empty()&&st.ep<(int)st.detail.episodes.size()){label=st.detail.episodes[st.ep].label;url=st.detail.episodes[st.ep].url;}text(b,40,145,29,st.detail.title+" · "+label,white,1160);rect(b,g_stride,40,190,1200,330,card);text(b,70,245,23,"播放器接口",pink,500);text(b,70,295,18,"当前版本完成目录、详情与选集。播放核心下一阶段接入。",muted,1100,2);text(b,70,405,16,url,muted,1100,2);footer(b,st,"B 返回");}
...
static bool resolve_title(State& st,size_t idx){if(idx>=st.items.size()||!unresolved(st.items[idx]))return true;AnimeDetail d;std::string m;if(provider_fetch_detail(st.proxy,st.items[idx],d,m)){if(!d.title.empty())st.items[idx].title=d.title;return true;}return false;}
static void resolve_visible(State& st,Framebuffer& fb){if(st.view.empty())return;int rows=7,start=st.selected>=rows?st.selected-rows+1:0,end=std::min((int)st.view.size(),start+rows);for(int vi=start;vi<end;++vi){size_t idx=st.view[vi];if(!unresolved(st.items[idx]))continue;char p[96];snprintf(p,sizeof(p),"正在读取番剧标题 %d/%d",vi-start+1,end-start);st.status=p;draw_frame(fb,st);resolve_title(st,idx);}rebuild(st);st.status="标题缓存完成";}
static void connect_source(State& st,Framebuffer& fb){ensure_network(st);if(!st.socket_ok||!st.provider_ok)return;std::vector<AnimeItem> out;std::string m;if(!provider_fetch_home(st.proxy,out,m)){st.status=m;return;}st.items.swap(out);st.connected=true;st.status=m;rebuild(st);std::string e;if(!init_hos_fonts(e)){st.status=m+" · HOS FONT FAILED: "+e;return;}st.screen=CATALOG;draw_frame(fb,st);resolve_visible(st,fb);st.status=m;}
static void refresh(State& st,Framebuffer& fb){std::vector<AnimeItem> out;std::string m;if(provider_fetch_home(st.proxy,out,m)){st.items.swap(out);st.selected=0;rebuild(st);resolve_visible(st,fb);st.status=m;}else st.status=m;}
static void open_detail(State& st){if(st.view.empty())return;size_t idx=st.view[st.selected];AnimeDetail d;std::string m;if(provider_fetch_detail(st.proxy,st.items[idx],d,m)){st.detail=std::move(d);if(!st.detail.title.empty())st.items[idx].title=st.detail.title;st.ep=0;st.screen=DETAIL;}st.status=m;}
static void do_search(State& st){std::string q=st.search;if(!keyboard("搜索番剧","搜索已缓存的番剧标题；留空显示全部",q,q))return;st.search=q;st.selected=0;rebuild(st);st.status=q.empty()?"已清除搜索":("搜索："+q);}

int main(int argc,char** argv){(void)argc;(void)argv;mkdir(LOG_DIR,0777);FILE* lf=fopen(LOG_PATH,"w");if(lf){fprintf(lf,"NXAnime 0.2.1 startup\n");fclose(lf);}log_stage("main entered");padConfigureInput(1,HidNpadStyleSet_NpadStandard);PadState pad;padInitializeDefault(&pad);Framebuffer fb;Result frc=framebufferCreate(&fb,nwindowGetDefault(),W,H,PIXEL_FORMAT_RGBA_8888,2);if(R_FAILED(frc))return (int)frc;framebufferMakeLinear(&fb);State st;proxy_config_load(&st.proxy);draw_frame(fb,st);
    while(appletMainLoop()){padUpdate(&pad);u64 d=padGetButtonsDown(&pad);if(d&HidNpadButton_Plus)break;
        if(st.screen==SAFE){if(d&HidNpadButton_A){st.status="CONNECTING GIRIGIRI...";draw_frame(fb,st);connect_source(st,fb);}if(d&HidNpadButton_Y){ensure_network(st);if(st.socket_ok&&st.provider_ok){std::string m;bool ok=provider_test_source(st.proxy,m);st.status=(ok?"SOURCE TEST OK: ":"SOURCE TEST FAILED: ")+m;}}if(d&HidNpadButton_ZR){st.proxy.mode=(ProxyMode)(((int)st.proxy.mode+1)%3);proxy_config_save(&st.proxy);st.status=std::string("PROXY MODE: ")+proxy_mode_name(st.proxy.mode);}}
        else if(st.screen==CATALOG){if(!st.view.empty()){int old=st.selected;if(d&HidNpadButton_Up)st.selected=(st.selected-1+(int)st.view.size())%(int)st.view.size();if(d&HidNpadButton_Down)st.selected=(st.selected+1)%(int)st.view.size();if(st.selected!=old)resolve_visible(st,fb);if(d&HidNpadButton_A)open_detail(st);}if(d&HidNpadButton_X)do_search(st);if(d&HidNpadButton_Y)refresh(st,fb);}
        else if(st.screen==DETAIL){int n=st.detail.episodes.size();if(n>0){if(d&HidNpadButton_Left)st.ep=(st.ep-1+n)%n;if(d&HidNpadButton_Right)st.ep=(st.ep+1)%n;if(d&HidNpadButton_Up)st.ep=std::max(0,st.ep-8);if(d&HidNpadButton_Down)st.ep=std::min(n-1,st.ep+8);if(d&HidNpadButton_A)st.screen=EPISODE;}if(d&HidNpadButton_B)st.screen=CATALOG;}
        else if(st.screen==EPISODE){if(d&HidNpadButton_B)st.screen=DETAIL;}
        draw_frame(fb,st);svcSleepThread(12'000'000);
    }
    if(st.provider_ok)provider_exit();if(st.socket_ok)socketExit();shutdown_fonts();framebufferClose(&fb);return 0;
}
