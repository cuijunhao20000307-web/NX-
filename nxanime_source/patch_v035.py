...
if old_enum not in m: raise SystemExit('screen enum not found')
...
if old_rebuild not in m: raise SystemExit('rebuild block not found')
...
search_draw = r'''static void draw_search(u32* b,const State& st){
    ...
    text(b,70,505,16,"系统中文键盘内按 HOS 取消键回到此页；此页按 X 立即返回。",muted,1130,2);
    footer(b,st,"A 输入   X 返回   Y 首页");
}

static void draw_filter(u32* b,const State& st){'''
if marker not in m: raise SystemExit('draw_filter marker not found')
...
if old_draw_frame not in m: raise SystemExit('draw_frame filter branch not found')
...
if old_refresh not in m: raise SystemExit('refresh block not found')
m = m.replace(old_refresh, new_refresh, 1)

start = m.find('static void do_search(State& st,Framebuffer& fb){')
if start < 0: raise SystemExit('do_search start not found')
end = m.find('\nstatic bool hit_box', start)
if end < 0: raise SystemExit('do_search end marker not found')
new_search_block = r'''static void execute_search(State& st,Framebuffer& fb){
    ...
    if(!keyboard("搜索番剧","输入中文片名；取消会回到 NXAnime 搜索页",q,q)){
        st.status="已取消输入";st.screen=SEARCH;return;
    }
    st.search=q;st.selected=0;st.cover.clear();st.cover_id.clear();
    if(q.empty()){return_home(st,fb);return;}
    std::vector<AnimeItem> out;std::string msg;
    st.status="正在搜索："+q;draw_frame(fb,st);
...
insert_touch = r'''    if(st.screen==SEARCH){
    ...
    if(st.screen==FILTER){'''
if marker_touch not in m: raise SystemExit('touch filter marker not found')
m = m.replace(marker_touch, insert_touch, 1)

# Replace the old touch-footer search call left by v0.3.3.
m = m.replace('do_search(st,fb);', 'st.screen=SEARCH;')
...
# Depending on the previous replacement, the first line may already be SEARCH.
...
old_catalog_after = old_catalog.replace('do_search(st,fb);','st.screen=SEARCH;')
if old_catalog_after not in m: raise SystemExit('catalog input block not found')
m = m.replace(old_catalog_after, new_catalog, 1)
